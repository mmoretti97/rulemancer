...
\begin{frame}[fragile]{Running the Server}
...
% \begin{frame}{Defining a New Game}
% \begin{itemize}
%   \item Create metadata \texttt{.clp} in \texttt{rulepool/}.
%   \item Required templates:
%   \begin{itemize}
%     \item \texttt{game-config}
%     \item \texttt{assertable}
%     \item \texttt{results}
%     \item \texttt{queryable}
%   \end{itemize}
%   \item Then add game-specific rules/templates for internal logic.
% \end{itemize}
% \end{frame}

% \begin{frame}{Interface Contract in CLIPS}
% \begin{itemize}
%   \item \texttt{assertable} defines accepted external actions.
%   \item \texttt{results} defines returned facts after processing.
%   \item \texttt{queryable} defines readable game state.
%   \item Rulemancer turns these contracts into REST interaction surfaces.
% \end{itemize}
% \end{frame}

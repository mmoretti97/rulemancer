...
The server exposes a REST API under the \texttt{/api/v1} route, which is organized into several sub-routes
...
\begin{itemize}
  ...
  \vspace{0.3cm}
  \item \texttt{/bridge} and \texttt{/brroom} $\rightarrow$ direct JSON-to-CLIPS bridge workflows.
  \vspace{0.3cm}
  \item \texttt{/join} and \texttt{/watch} $\rightarrow$ room participation workflows.
\end{itemize}
\end{frame}

\begin{frame}{Bridge Mode: Direct JSON $\leftrightarrow$ CLIPS}
Bridge mode is designed for generic integrations where clients do not need game-specific
\texttt{assertable/results/queryable} contracts.\\
\vspace{0.3cm}
Flow:
\begin{itemize}
  \item Configure bridges in \texttt{rulemancer.json}: \texttt{"bridges": \{"name": "rulepool/path"\}}.
  \item Create a bridge room: \texttt{POST /api/v1/brroom/create}.
  \item Send facts + queries in one call: \texttt{POST /api/v1/brroom/\{id\}/request}.
  \item Receive asserted CLIPS facts plus queried relations in a structured JSON response.
\end{itemize}
\vspace{0.3cm}
Useful endpoints:
\begin{itemize}
  \item \texttt{GET /api/v1/bridge/list}, \texttt{GET /api/v1/bridge/\{id\}}
  \item \texttt{GET /api/v1/brroom/list}, \texttt{DELETE /api/v1/brroom/\{id\}}
\end{itemize}
\end{frame}

\begin{frame}[fragile]{Bridge Request Example}
\textbf{Request}
\begin{verbatim}
POST /api/v1/brroom/bridge-session-1/request
{
  "facts": [
    {"first": {"x": ["a"], "y": ["v"]}}
  ],
...
\begin{frame}{Game and Room Endpoints (Some)}
Th admin JWT can be used to access system-level endpoints for managing games and rooms, such as listing available games, 
inspecting game schemas, creating new rooms, and listing/inspecting existing rooms. Here are some examples: \\
\vspace{0.3cm}
\begin{itemize}
...
\end{itemize}
\vspace{0.3cm}
Scripts are provided in the generated interface files to perform these actions with cURL, using the admin token for authentication.
\end{frame}

\begin{frame}{Joining and Watching Rooms}
Players can join rooms using the \texttt{/join} endpoint, which assigns them to an available room of the specified game and returns the room context and player token. \\
\vspace{0.3cm}
Watchers can connect to rooms using the \texttt{/watch} endpoint, which allows them to receive live updates from the room without participating as players. \\
\vspace{0.3cm}
Example endpoints:
\begin{itemize}
  \item Join room: \texttt{POST /api/v1/join/available/\{gameName\}}
  \item Watch room: \texttt{POST /api/v1/room/\{id\}/watch}
\end{itemize}
\vspace{0.3cm}
The generated interface scripts include cURL commands for these interactions, which handle the necessary authentication and context management for players and watchers.
\end{frame}

\begin{frame}{Assert and Query Endpoints (Players and Watchers)}
Players and watchers can interact with rooms through the assert and query endpoints,
which allow them to perform game actions and retrieve game state information according to the defined CLIPS rules. \\
\vspace{0.3cm}
\begin{itemize}
  \item Assert action:
  \begin{itemize}
    \item \texttt{POST /api/v1/room/\{id\}/assert/\{action\}}
    \item Response envelope format: \texttt{\{ "response": \{ ... \} \}}
  \end{itemize}
  \item Query state:
  \begin{itemize}
    \item \texttt{POST /api/v1/room/\{id\}/query/\{query\}}
  \end{itemize}
\end{itemize}
\vspace{0.3cm}
Also in this case, the generated interface scripts include cURL commands 
for performing these interactions based on the CLIPS-defined assertable and queryable facts for each game.
\end{frame}

\begin{frame}{Real-Time Communication}
The client interactions described so far are based on a request-response model using REST API,
which is suitable for most game actions and queries. However, for real-time updates and monitoring, 
Rulemancer also provides WebSocket endpoints that clients can connect to for live streaming of events
and state changes. \\
\vspace{0.3cm}
\begin{itemize}
  ...
  \item Room websocket:
  \begin{itemize}
    \item \texttt{wss://localhost:3000/api/v1/room/\{id\}/ws} (players and watchers)
  \end{itemize}
...
\begin{frame}{Web Interface (Built-In for Game Development and Debugging)}
The engine includes a built-in web interface for monitoring and interacting with games, 
which is automatically generated from templates at startup. \\
\vspace{0.2cm}
Characteristics of the web interface: 
\vspace{0.2cm}
\begin{itemize}
  \item Route family mounted at \texttt{/api/v1/web}.
  \vspace{.2cm}
  \item Dynamic pages are generated from templates at startup according to the defined assertable/queryable facts in the CLIPS rules.
  \vspace{.2cm}
  \item Admin page and game-specific client pages are served directly by backend.
  \vspace{.2cm}
  \item Admin page path: \texttt{/api/v1/web/admin/dashboard}.
  \vspace{.2cm}
  \item Game chooser page at  \texttt{/api/v1/web/client/chooser}, Example game page path: \texttt{/api/v1/web/client/\{gameName\}}.
\end{itemize}
\end{frame}

\begin{frame}{Admin Dashboard Capabilities}
\small
The admin dashboard provides a comprehensive overview of the server's state and activity, including:
\begin{columns}
...
\begin{itemize}
  ...
  \item Connect to system websockets for live monitoring of server events.
\end{itemize}
\end{column}
\begin{column}{0.7\textwidth}
\begin{center}
\includegraphics[width=0.9\textwidth]{admindashboard.png}
\end{center}
\end{column}
\end{columns}
\end{frame}
...
\small
The game client pages provide an interface for players and watchers to interact with specific games and rooms. This 
is useful for rapid prototyping and debugging game interfaces. \\
\begin{columns}
\begin{column}{0.3\textwidth} 
\begin{itemize}
...
\begin{column}{0.7\textwidth}
\begin{center}
\includegraphics[width=0.9\textwidth]{gamedashboard.png}
\end{center}
\end{column}
\end{columns}
\end{frame}
...
\textbf{Assert move (tic-tac-toe example)}
\begin{verbatim}
